...
A pnpm workspace now exists with two implemented packages — `packages/engine-core` (the backup engine, pure TypeScript, zero UI dependency) and `packages/engine-cli` (a commander CLI wrapping it). Two of the three backup strategies are implemented and verified end-to-end against real local targets (Windows OpenSSH Server): **`fetch_existing`** (SFTP: detect an existing remote dump → download → validate → record) and **`remote_dump`** (SSH: run a configured remote command to generate the dump → download → optionally delete the remote file → validate → record). Verified behaviors include host-key confirmation, incremental checksumming, `.part`-then-rename handling, no redundant re-downloads (fetch_existing), correct nonzero-exit-code failure handling (remote_dump), interrupted-run recovery, and the app-level concurrency lock. `direct_dump` is still a stub. `packages/ui` and `packages/desktop-shell` (Tauri) do not exist yet — see "Not yet built" below.
...
#   | task:create [--strategy fetch_existing|remote_dump] | task:run <taskId>
#   | task:test-connection <taskId> | status [--json]
```
...
The spec explicitly says: **do not build the whole app at once**, and the user wants to be **contradicted** when a technical decision in the spec doesn't hold up, not just agreed with. This governed the architecture phase (already done — see "Architecture" below) and still governs what comes next: **no feature beyond the currently-agreed increment without explicitly checking in first.** `fetch_existing` and `remote_dump` are done; don't start on retention, `direct_dump`, Task Scheduler registration, automated tests, or the GUI without asking which is next — each is a deliberate, separately-agreed increment, not a natural continuation to pick up unprompted. (This has held in practice: increments so far were each chosen via an explicit question to the user before starting.)
...
- **`direct_dump` — stub only** (`strategies/directDumpExecutor.ts`): `produce()` throws "not implemented." Needs a `DatabaseDumpClient` (interface already defined in `databaseConnections/types.ts`, no implementation) and will require bundling `pg_dump` locally (fine, permissive license) and, for MySQL, `mysqldump` (GPLv2 — the same bundling decision already deferred for validation resurfaces here for dump *generation*). Do not implement without an explicit go-ahead.
