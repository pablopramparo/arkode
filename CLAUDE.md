...
Everything below is real product scope from `project.md` but has no implementation yet — don't assume it exists, and don't build it without confirming the increment first: a GUI of any kind (`packages/ui`, `packages/desktop-shell` don't exist — Tauri hasn't been scaffolded), and packaging/distribution (no installer, no vendored `pg_dump.exe`/`pg_restore.exe`/`mysqldump.exe`, no compiled `engine-cli.exe` via `@yao-pkg/pkg`, no resolved decision on bundling GPL-licensed `mysqldump` — and now also no update to `scheduler:install`'s Action command for whatever that compiled exe's install path turns out to be). The CLI's `status --json` command is a primitive for a future dashboard to consume, not the dashboard itself — note it currently reports the single *latest* run per task, so a no-op "already up to date" run can show `sizeBytes`/`checksumSha256` as `null` even though a prior run has real data; a real dashboard will need to look further back for that. Also not done: an actual live `scheduler:install` registration with a real Windows password (verified only up to the credential-check stage in this session — see "Scheduler").
