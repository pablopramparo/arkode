...
export function IconButton({
  ...
  disabled?: boolean;
  tone?: 'muted' | 'danger';
  /** Transient "done" state (e.g. just copied) — swaps in a green check so the feedback is visible, not only in the tooltip. */
  confirmed?: boolean;
}) {
  return (
    <button
      ...
      style={{ color: confirmed ? 'var(--success)' : tone === 'danger' ? 'var(--danger)' : 'var(--muted)' }}
      onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = 'var(--surface-secondary)')}
      onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'transparent')}
    >
      <span className="h-4 w-4 [&>svg]:h-4 [&>svg]:w-4">{confirmed ? <CheckCircleIcon /> : icon}</span>
    </button>
  );
}
